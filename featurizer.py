...
def atom_represent(df, num_ele=3, num_sites=20):
    Element = joblib.load('./files/element.pkl')
    ...
    for idx in range(len(df)):  # 46382
        ...
        #         print(ratio.shape,fc1_ind.shape,crys_list1.shape)
        ...
        # real space represeatnion
        atom_list = np.concatenate((onehot, ratio.T, ele_feature, np.zeros((num_ele, 1)), coeffs_crsytal), axis=1)
...
def  dissimilarity(df,name):
    ...
    v_new =[]
    for j in name:
        new_crystal=Structure.from_file('D'+str(j)+'.cif')
    ...
    for idx in range(len(df)): #46382
        ...
        v_crystal =  np.array(ssf.featurize(crystal))
        
        dis = []
        
        for v in v_new:
          
            dis.append(np.linalg.norm(v_crystal - v))
        
        total.append(np.array(dis))
...
def plot_confusion_matrix(model_, x_data, y_data, file_name):
    ...
    print('Precision:', cm2[1, 1] / (cm2[1, 1] + cm2[0, 1]))
    print('Recall:', cm[1, 1])
